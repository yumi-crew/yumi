...
{ 
  ...
  for (size_t i = 0; i < registered_joint_state_handles_.size(); i++)
  {
    ...
    if (joint_cmd_iter != registered_joint_cmd_handles_.cend())
    {
      ...
      {
        // RCLCPP_INFO(this->get_lifecycle_node()->get_logger(), "joint: %s", joint_name.c_str());
        // RCLCPP_INFO(this->get_lifecycle_node()->get_logger(), "curr_pos: %f ", angles::to_degrees(curr_pos));
        // RCLCPP_INFO(this->get_lifecycle_node()->get_logger(), "desired_pos: %f", angles::to_degrees(desired_pos)); 
        //--------------PID---------------------------------------------------------------------------------------------
        auto error = desired_pos - curr_pos;
        auto addition = pid_controllers_map_[joint_name]->compute_command(error, timeElapsed);
        //--------------------------------------------------------------------------------------------------------------
        // RCLCPP_INFO(this->get_lifecycle_node()->get_logger(), "Sending command");
