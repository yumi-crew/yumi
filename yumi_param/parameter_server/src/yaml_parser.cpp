...
void get_key_values( YAML::Node root, const std::string& key, std::unordered_map<std::string, std::string> &key_values) 
{
  ...
  if (root.Type() == YAML::NodeType::Map) 
  {
    ...
    {
      get_key_values(root_it->second, key + separator + root_it->first.as<std::string>(), key_values);
    }
  }
...
void YamlParser::parse(const std::string &absolute_file_path) 
{
...
std::unordered_map<std::string, std::string>  YamlParser::get_key_value_pairs() 
{
