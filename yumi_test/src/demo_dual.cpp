...
int main(int argc, char **argv)
{
  ...
  std::vector<std::string> objects = {"small_marker"};
  double percentage = 0;
  int lin_retries = 3;
  ...
  yumi_motion_coordinator->move_to_state("left_arm", {-0.61, -1.95,  0.94, 0.79, -1.16, 0.27, 3.29});
  ...
  est_success = pose_estimation_manager->call_estimate_pose_srv(pick_bin, 0, 100s, "", 0.0, true);
  ...
  while (counter < num_picks)
  {
    for (auto object : objects)
    {
      ...
      yumi_motion_coordinator->move_to_state("right_arm", {0.43, -1.32, -0.61, 0.64, -1.44, 1.27, -0.07});
      yumi_motion_coordinator->move_to_state("left_arm", {-0.61, -1.95,  0.94, 0.79, -1.16, 0.27, 3.29});
      ...
      // Left arm goes to serving position
      ...
      // Place at a object at "serving tray"
      yumi_motion_coordinator->move_to_state("right_arm", {1.71, -0.33, -1.57, 0.74, 1.40, -1.13, 1.72}, 5);
      yumi_motion_coordinator->drop_object(object, "right_arm");

      // Right arm moves async away from camera view (same as start) (async)
      yumi_motion_coordinator->move_to_state("right_arm", {0.43, -1.32, -0.61, 0.64, -1.44, 1.27, -0.07}, 5, false, false);
      sleep(1);

      // Left arm moves object to bin and rotates wrist.
      yumi_motion_coordinator->move_to_state("left_arm", {-1.32, -1.7, 1.45, 0.59, 0.8, 0.74, 1.98}, 5, false, true);
      yumi_motion_coordinator->move_to_state("left_arm", {-1.32, -1.7, 1.45, 0.59, 0.8, 0.74, 0.32}, 5, false, true);
